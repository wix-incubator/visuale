...
export class ActionMenu extends React.Component<IActionMenuProps> {
    public render() {
        ...
        return (<div id="popper" style={{ background: "#ddd", padding: "10px" }} data-component="_">
        <h3>{component.name}</h3>
        {component.actions.map((action: IAction) => <button onClick={() => this.recordAction(action.name)} key={action.name}>{action.name}</button>)}
      </div>);
    }
    private recordAction = (actionName: string) => {
        ...
        if (action.args.length) {
            parameters = action.args.map((arg) => (prompt(`${arg.name}: ${arg.type}`)) || "");
        }
